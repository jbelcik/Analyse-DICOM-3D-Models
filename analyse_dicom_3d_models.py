...
class Navigation(bpy.types.Panel):
	...
	def draw(self, context):
		...
		box = layout.box()
		box.label(text = 'Orbit the view around:')
		...
		box = layout.box()
		box.label(text = 'Viewpoint:')
		
		col = box.column(align = True)
		
		row = col.row(align = True)
		row.operator('opr.left_viewpoint',	text = 'Left')
		row.operator('opr.right_viewpoint',	text = 'Right')
		
		row = col.row(align = True)
		row.operator('opr.front_viewpoint',	text = 'Front')
		row.operator('opr.back_viewpoint',	text = 'Back')
		
		row = col.row(align = True)
		row.operator('opr.top_viewpoint',		text = 'Top')
		row.operator('opr.bottom_viewpoint',	text = 'Bottom')
		
		col.operator('view3d.view_persportho', text = 'Persp/Ortho')
		
		if context.object != None:
			if not context.object.hide:
				col.operator('view3d.view_selected', text = 'View Selected')
...
class Tools(bpy.types.Panel):
	...
	def draw(self, context):
		...
		col = spl.column()
		col.operator('ed.undo', icon = 'BACK')
		col = spl.column()
		col.operator('ed.redo', icon = 'FORWARD')
		
		layout.operator('opr.import_ply_object', text = 'Import Stanford (.ply)',	icon = 'FILE')
		layout.operator('opr.import_stl_object', text = 'Import Stl (.stl)',		icon = 'FILE')
		
		if context.object != None:
			if context.object.type == 'MESH':
				if context.object.mode == 'OBJECT':
					if not context.object.hide:
						layout.operator('opr.enter_edit_mode', icon = 'EDITMODE_HLT')
						layout.operator('opr.fix_origin', text = 'Origin To Geometry', icon = 'ROTATE')
						
				if context.object.mode == 'EDIT':
					layout.operator('opr.enter_object_mode', icon = 'OBJECT_DATAMODE')
					
				if not context.object.hide:
					obj_name = context.object.name
					obj = bpy.data.objects[obj_name]
					
					if obj.modifiers.find('DCIM.Decimate') == -1:
						layout.operator('opr.add_decimate_modifier', text = 'Add Decimate', icon = 'MOD_DECIM')
					else:
						box = layout.box()
						box.label(text = 'Decimate', icon = 'MOD_DECIM')
						col = box.column()
						row = col.row()
						
						rowr = row.row(align = True)
						rowr.prop(obj.modifiers['DCIM.Decimate'], 'show_render',	text = '')
						rowr.prop(obj.modifiers['DCIM.Decimate'], 'show_viewport',	text = '')
						
						rowr = row.row(align = True)
						
						rowr = row.row(align = True)
						rowr.operator('opr.move_up_decimate_modifier',		text = '', icon = 'TRIA_UP')
						rowr.operator('opr.move_down_decimate_modifier',	text = '', icon = 'TRIA_DOWN')
						
						rowr = row.row(align = True)
						rowr.operator('opr.remove_decimate_modifier', text = '', icon = 'X')
						
						row = col.row()
						row.label(text = 'Position in the stack: ' + str(obj.modifiers.find('DCIM.Decimate') + 1))
						
						row = col.row()
						row.prop(obj.modifiers['DCIM.Decimate'], 'decimate_type')
						
						if context.object.modifiers['DCIM.Decimate'].decimate_type == 'COLLAPSE':
							col.prop(obj.modifiers['DCIM.Decimate'], 'ratio')
							col.prop(obj.modifiers['DCIM.Decimate'], 'use_collapse_triangulate')
							col.label(text = 'Face count: ' + str(obj.modifiers['DCIM.Decimate'].face_count))
							
						if context.object.modifiers['DCIM.Decimate'].decimate_type == 'UNSUBDIV':
							col.prop(obj.modifiers['DCIM.Decimate'], 'iterations')
							col.label(text = 'Face count: ' + str(obj.modifiers['DCIM.Decimate'].face_count))
							
						if context.object.modifiers['DCIM.Decimate'].decimate_type == 'DISSOLVE':
							col.prop(obj.modifiers['DCIM.Decimate'], 'angle_limit')
							col.prop(obj.modifiers['DCIM.Decimate'], 'use_dissolve_boundaries')
							col.label(text = 'Delimit:')
							row = col.row()
							row.prop(obj.modifiers['DCIM.Decimate'], 'delimit')
							col.label(text = 'Face count: ' + str(obj.modifiers['DCIM.Decimate'].face_count))
						...
						if context.object.mode == 'OBJECT':
							...
							colc = spl.column()
							colc = spl.column()
							colc.operator('opr.apply_decimate_modifier', text = 'Apply')
							
					if obj.modifiers.find('DCIM.Smooth') == -1:
						layout.operator('opr.add_smooth_modifier', text = 'Add Smooth', icon = 'MOD_SMOOTH')
					else:
						box = layout.box()
						...
						rowr = row.row(align = True)
						rowr.prop(obj.modifiers['DCIM.Smooth'], 'show_render', text = '')
						rowr.prop(obj.modifiers['DCIM.Smooth'], 'show_viewport', text = '')
						rowr.prop(obj.modifiers['DCIM.Smooth'], 'show_in_editmode', text = '')
						rowr.prop(obj.modifiers['DCIM.Smooth'], 'show_on_cage', text = '')
						
						rowr = row.row(align = True)
						rowr.operator('opr.move_up_smooth_modifier',	text = '', icon = 'TRIA_UP')
						rowr.operator('opr.move_down_smooth_modifier',	text = '', icon = 'TRIA_DOWN')
						
						rowr = row.row(align = True)
						rowr.operator('opr.remove_smooth_modifier', text = '', icon = 'X')
						
						row = col.row()
						row.label(text = 'Position in the stack: ' + str(obj.modifiers.find('DCIM.Smooth') + 1))
						
						spl = col.split(0.25)
						colc = spl.column()
						colc.label(text = 'Axis:')
						colc.prop(obj.modifiers['DCIM.Smooth'], 'use_x')
						colc.prop(obj.modifiers['DCIM.Smooth'], 'use_y')
						colc.prop(obj.modifiers['DCIM.Smooth'], 'use_z')
						
						colc = spl.column()
						colc.prop(obj.modifiers['DCIM.Smooth'], 'factor')
						colc.prop(obj.modifiers['DCIM.Smooth'], 'iterations')
						
						if context.object.mode == 'OBJECT':
							col = box.column()
							spl = col.split(0.73)
							colc = spl.column()
							colc.operator('opr.apply_as_shape_smooth_modifier', text = 'Apply as Shape Key')
							colc = spl.column()
							colc.operator('opr.apply_smooth_modifier', text = 'Apply')
							
		if context.object != None:
			if context.object.type == 'MESH':
				if not context.object.hide and context.object.mode in ['OBJECT', 'EDIT']:
					box = layout.box()
					
					col = box.column()
					spl = col.split(0.8)
					col = spl.column()
					col.label(text = 'Trim', icon = 'META_CUBE')
					col = spl.column()
					
					if context.scene.trim_expand:
						col.operator('opr.trim_expand', text = '', icon = 'TRIA_DOWN')
						
						col = box.column()
						col.prop(context.scene, 'trim_origin_translation')
						
						col = box.column()
						col.prop(context.scene, 'trim_use_fill')
						
						col = box.column(align = True)
						col.label(text = 'X Axis Interval:')
						col.prop(context.scene, 'trim_x_start')
						col.prop(context.scene, 'trim_x_end')
						
						col = box.column(align = True)
						col.label(text = 'Y Axis Interval:')
						col.prop(context.scene, 'trim_y_start')
						col.prop(context.scene, 'trim_y_end')
						
						col = box.column(align = True)
						col.label(text = 'Z Axis Interval:')
						col.prop(context.scene, 'trim_z_start')
						col.prop(context.scene, 'trim_z_end')
						
						col = box.column()
						spl = col.split(0.5)
						col = spl.column()
						col.operator('opr.save_trimmed', text = 'Save as New')
						col = spl.column()
						col.operator('opr.trim_object', text = 'Apply')
					else:
						col.operator('opr.trim_expand', text = '', icon = 'TRIA_LEFT')
				...
				spl = col.split(0.8)
				col = spl.column()
				col.label(text = 'Intersect', icon = 'MOD_BEVEL')
				col = spl.column()
				
				if context.scene.intersect_expand:
					col.operator('opr.intersect_expand', text = '', icon = 'TRIA_DOWN')
					
					col = box.column()
					col.prop(context.scene, 'intersect_hide')
					
					col = box.column()
					spl = col.split(0.5)
					col = spl.column()
					col.operator('opr.show_prev_intersection', text = 'Prev', icon = 'BACK')
					col = spl.column()
					col.operator('opr.show_next_intersection', text = 'Next', icon = 'FORWARD')
					
					if not context.object.hide and context.object.mode in ['OBJECT', 'EDIT']:
						col = box.column()
						col.label(text = 'Origin Location:')
						col.prop(context.scene, 'intersect_origin', text = '')
						
						col = box.column()
						col.label(text = 'Plane Orientation:')
						col.prop(context.scene, 'intersect_plane', text = '')
						
						col = box.column()
						col.prop(context.scene, 'intersect_use_fill')
						col.prop(context.scene, 'intersect_clear_inner')
						col.prop(context.scene, 'intersect_clear_outer')
						
						col = box.column()
						spl = col.split(0.5)
						col = spl.column()
						col.operator('opr.save_intersection', text = 'Save as New')
						col = spl.column()
						col.operator('opr.intersect_object', text = 'Apply')
				else:
					col.operator('opr.intersect_expand', text = '', icon = 'TRIA_LEFT')
				...
				spl = col.split(0.8)
				col = spl.column()
				col.label(text = 'Dissect', icon = 'UV_EDGESEL')
				col = spl.column()
				
				if context.scene.dissect_expand:
					col.operator('opr.dissect_expand', text = '', icon = 'TRIA_DOWN')
					
					col = box.column()
					col.prop(context.scene, 'dissect_hide')
					
					col = box.column()
					spl = col.split(0.5)
					col = spl.column()
					col.operator('opr.show_prev_dissection', text = 'Prev', icon = 'BACK')
					col = spl.column()
					col.operator('opr.show_next_dissection', text = 'Next', icon = 'FORWARD')
					
					if not context.object.hide and context.object.mode in ['OBJECT', 'EDIT']:
						col = box.column()
						col.label(text = 'Origin Location:')
						col.prop(context.scene, 'dissect_origin', text = '')
						
						col = box.column()
						col.label(text = 'Plane Orientation:')
						col.prop(context.scene, 'dissect_plane', text = '')
						
						col = box.column()
						col.prop(context.scene, 'dissect_use_fill')
						
						col = box.column()
						col.prop(context.scene, 'dissect_uncut')
						
						col = box.column()
						col.prop(context.scene, 'dissect_step')
						
						col = box.column(align = True)
						col.label(text = 'Dissect Interval:')
						col.prop(context.scene, 'dissect_start')
						col.prop(context.scene, 'dissect_end')
						
						col = box.column()
						spl = col.split(0.73)
						col = spl.column()
						col = spl.column()
						col.operator('opr.dissect_object', text = 'Apply')
				else:
					col.operator('opr.dissect_expand', text = '', icon = 'TRIA_LEFT')
